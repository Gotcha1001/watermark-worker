...
//             const fontSize = Math.max(10, watermarkSize); // Use watermarkSize directly
//             const textWidth = watermarkText.length * fontSize * 0.6; // Match client
//             const textHeight = fontSize * 1.5; // Increased for ascent/descent
//             const svgWidth = Math.floor(Math.min(textWidth, metadata.width * 0.8)); // Cap at 80% image width
//             const svgHeight = Math.floor(Math.min(textHeight, metadata.height));

//             // Adjust for rotation bounding box
...
//             // Escape watermarkText to prevent XML parsing errors
...
//             console.log("Generated SVG:", svgText); // Debug SVG content
...
//             // Resize only if necessary, preserving text size
//             if (overlayMetadata.width > metadata.width || overlayMetadata.height > metadata.height) {
...
//             // Calculate logo size (match client: percentage of image width)
...
//             let logoWithAlpha = await sharp(logoSharp)
//                 .raw()
...
//             if (overlayMetadata.width > metadata.width || overlayMetadata.height > metadata.height) {
//                 console.log("Resizing logo overlay:", {
//                     original: { width: overlayMetadata.width, height: overlayMetadata.height },
//                     target: { width: metadata.width, height: metadata.height },
//                 });
//                 overlayBuffer = await sharp(overlayBuffer)
...
export async function POST(request) {
    try {
        ...
        if (watermarkType === "text") {
            ...
            const fontSize = Math.max(10, watermarkSize); // Use watermarkSize directly
            const textWidth = watermarkText.length * fontSize * 0.65; // Adjusted for Arial metrics
            const textHeight = fontSize * 1.5; // Matches client
            ...
            overlayBuffer = await sharp(Buffer.from(svgText))
                .png({ quality: 100, force: true }) // High-quality PNG
                .toBuffer();
